...
BRAKE_PHRASES = {
    "FRONT_ON":  [
        ...
        "on freni yap","on fireni yap","on frene vur","on freni tut","on freni tuttur",
        "ondeki freni ac","ileri freni ac","front freni ac",
    ],
    "REAR_ON":   [
        ...
        "arka freni yap","arka fireni yap","arka frene vur","arka freni tut","arka freni tuttur",
        "arkadaki fireni ac","geri freni ac","rear freni ac",
    ],
    "FRONT_OFF": [
        ...
        "on fireni birak","on firani birak","on freni iptal","ondeki freni birak",
    ],
    "REAR_OFF":  [
        ...
        "arka fireni birak","arka firani birak","arka freni iptal","orka freni birak",
    ],
    "ALL":       [
        "tum frenleri ac","hepsini ac","frenlere bas","tum frenlere bas",
        "butun frenleri ac","frenleri sik","tam fren","her iki freni ac",
        "fren yap","firen yap","frenle","freni cek","frenleri devreye al","frenleri kilitle",
        "iki freni de ac","her iki frene bas","frenleri uygula","frenleri vur",
        "frenleri tut","frenleri tuttur","frenleme yap","tam frenleme",
    ],
    "RELEASE":   [
        ...
        "fireni birak","firenle birak","freni iptal","freni serbest birak",
    ],
}
...
def detect_vehicle_command(text: str) -> Optional[str]:
    ...
    if has(["motor","motoru","motori","motore","motur","moter"]):
        # Durum sorgusu
        if has(["durum","nasil","calisiyor","calisiyor mu","kontrol","aktif mi","kapali mi",
                "ne yapiyor","acik mi","var mi","bakiyor","izle"]):
            return "MOTOR_STATUS"
        # Motor ON
        if has(["calistir","baslat","devreye","sok","aktif","hazirla","yak",
                "ver","start","basla","calismaya","devret","guc ver","koy"]) or has_w({"ac","on"}):
            return "MOTOR_ON"
        # Motor OFF
        if has(["durdur","kapat","bitir","pasif","devre disi","sondur","sustur",
                "sonlandir","kes","off","stop"]) or has_w({"dur"}):
            return "MOTOR_OFF"
    ...
    is_off   = has(["kapat","birak","birakin","serbest","gevset","kaldir",
                    "geri al","coz","cikar","bosalt","birakiyor","iptal"])
